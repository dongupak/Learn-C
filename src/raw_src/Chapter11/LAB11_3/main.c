...
// 최고 점수를 가진 학생 찾기 함수
Student find_highest(Student students[], int size) {
    Student top = students[0];  // 첫 번째 학생을 최고 점수로 가정
...
int main() {
    Student students[STUDENT_COUNT];  // 구조체 배열 선언
    ...
    // 최고 성적 학생 찾기
    Student highest = find_highest(students, STUDENT_COUNT);
    printf("\n===== 최고 성적 학생 =====\n");
